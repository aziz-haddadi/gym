...
export const api = {
  ...
  workoutTemplates: (includeArchived = false) => request(`/workout-templates?include_archived=${includeArchived}`),
  createWorkoutTemplate: (data) => request("/workout-templates", { method: "POST", body: data }),
  updateWorkoutTemplate: (id, data) => request(`/workout-templates/${id}`, { method: "PATCH", body: data }),
  updateWorkoutTemplateExercises: (id, exercises) => request(`/workout-templates/${id}/exercises`, {
    method: "PUT",
    body: { exercises },
  }),
  archiveWorkoutTemplate: (id) => request(`/workout-templates/${id}`, { method: "DELETE" }),
  ...
  activateProgram: (id, startsOn = null) => request(`/programs/${id}/activate`, {
    method: "POST",
    body: startsOn ? { starts_on: startsOn } : {},
  }),
